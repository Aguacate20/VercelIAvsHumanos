...
import { createInitialRecord, findParticipantByDocument, getParticipantProgress } from '@/lib/supabase'
...
export default function LandingPage() {
  ...
  async function handleStart() {
    ...
    setLoading(true)
    try {
      // Guardar inmediatamente en Supabase al hacer clic en "Comenzar"
      const participantId = await createInitialRecord({
        consentimiento: 'Sí',
      ...
      sessionStorage.setItem('document_id', documentId.trim())
  ...
  async function handleResume() {
    setError('')
    if (!documentId.trim() || !/^\d+$/.test(documentId)) {
      setError('Ingresa tu número de documento para continuar donde lo dejaste.')
      return
    }
    ...
    try {
      const participant = await findParticipantByDocument(documentId.trim())
      if (!participant) {
        setError('No encontramos un registro previo con ese documento.')
        setResumeLoading(false)
        return
      }
      const progress = await getParticipantProgress(participant.id)
      sessionStorage.setItem('participant_id', participant.id)
      sessionStorage.setItem('participant_name', participant.name ?? '')
      sessionStorage.setItem('document_id', documentId.trim())
      sessionStorage.setItem('resume_from', String(progress))
      router.push('/experiment')
    } catch {
      setError('Error al buscar tu progreso. Intenta de nuevo.')
      setResumeLoading(false)
    }
  }
        ...
        <div className="w-full max-w-lg">
          ...
          <div className="card p-6 md:p-8 fade-in" style={{ animationDelay: '0.1s' }}>
            ...
            <div className="mb-6">
              ...
              <div className="bg-sabana-blue-muted rounded-xl p-4 max-h-48 overflow-y-auto mb-4">
                <p className="text-xs text-gray-600 font-body leading-relaxed">
                  Muchas gracias por participar en el proyecto del Doctorado en Psicología acerca de decisiones emocionales y morales e Inteligencia Artificial. El presente cuestionario busca conocer sus datos sociodemográficos con el fin de generar un perfil completo de los participantes.<br /><br />
                  Toda la información aquí consolidada se manejará bajo principios de <strong>confidencialidad y anonimato</strong>. Los datos se codificarán para que sea imposible el manejo por parte de terceros ajenos al estudio.<br /><br />
            ...
            <button
              onClick={handleStart}
              disabled={loading}
              className="w-full py-3.5 rounded-xl font-body font-semibold text-white text-sm transition-all duration-200 disabled:opacity-60 disabled:cursor-not-allowed mb-3"
            ...
            <button
              ...
              className="w-full py-3 rounded-xl font-body font-medium text-sabana-blue text-sm border-2 border-sabana-blue/30 hover:border-sabana-blue/60 hover:bg-sabana-blue-muted transition-all duration-200 disabled:opacity-50"
            >
              {resumeLoading
                ? 'Buscando tu progreso...'
                : `↩ Continuar donde lo dejé (situación ${situations.length} posibles)`}
            </button>
          </div>
